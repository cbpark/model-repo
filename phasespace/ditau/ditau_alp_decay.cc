...
int main(int argc, char *argv[]) {
    ...
    for (auto iev = 1; !lhe.empty();
        ...
        auto elec = lhef::Particle(
            11 * std::abs(decayed_tau.pid()) / decayed_tau.pid(), 1,
            undecayed_tau_pos->first, undecayed_tau_pos->first, 0, 0,
            decay_prod->Px(), decay_prod->Py(), decay_prod->Pz(),
            decay_prod->E(), decay_prod->M(), 0, 0);
        entries.insert({9, elec});
        decay_prod = tau.GetDecay(1);
        auto alp = lhef::Particle(
            40, 1, undecayed_tau_pos->first, undecayed_tau_pos->first, 0, 0,
            decay_prod->Px(), decay_prod->Py(), decay_prod->Pz(),
            decay_prod->E(), decay_prod->M(), 0, 0);
