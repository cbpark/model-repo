...
const char APPNAME[] = "ditau_1prong_alp";
...
int main(int argc, char *argv[]) {
    ...
    const double MElecNuNu[3] = {MELEC, 0.0, 0.0};
    ...
    for (auto iev = 1; iev != nevent + 1; ++iev) {
        ...
        tau2_decay.SetDecay(*tau2, 3, MElecNuNu);  // --> e + nu + nu
        ...
        tau2_decay.Generate();
        elec = tau2_decay.GetDecay(0);
        // line 7
        ps.push_back(lhef::Particle(-11, 1, 4, 4, 0, 0, elec->Px(), elec->Py(),
                                    elec->Pz(), elec->E(), elec->M(), 0, 0));
        auto nu = tau2_decay.GetDecay(1);
        // line 8
        ps.push_back(lhef::Particle(12, 1, 4, 4, 0, 0, nu->Px(), nu->Py(),
                                    nu->Pz(), nu->E(), nu->M(), 0, 0));
        nu = tau2_decay.GetDecay(2);
        // line 9
        ...
        lhef::EventInfo evinfo(9, 1, +7.4364437e-02, 1.05830100e+01,
