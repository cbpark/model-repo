...
int charge(const PseudoJet& j) {
    ...
    if (id == -11 || id == -13 || id == 211 || id == 321 || id == 2212) {
        return 1;
    } else if (id == 11 || id == 13 || id == -211 || id == -321 ||
               id == -2212) {
        return -1;
    }
    return 0;
...
std::pair<PseudoJets, std::unique_ptr<ClusterSequence>> jetObjects(
    const HadronLevelData& hadrons, double coneR) {
    PseudoJets particles;
    for (const auto& h : hadrons.others) { particles.push_back(py2FjInput(h)); }

    fastjet::JetDefinition jetDef(fastjet::antikt_algorithm, coneR,
                                  fastjet::E_scheme, fastjet::Best);
    auto clusterSeq = std::make_unique<ClusterSequence>(particles, jetDef);
...
JetLevelData reconstructObjects(const HadronLevelData& hadronData,
    ...
    auto jetobjs = jetObjects(hadronData, coneR);
    auto jets = jetobjs.first;
    for (auto& j : jets) { j.set_user_index(0); }

    // b tagging.
    bTagging(&jets, hadronData.bPartons, coneR);
